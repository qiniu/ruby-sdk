...
class PutPolicy
  ...
  attr_accessor :scope, :callback_url, :callback_body, :return_url, :return_body, :persistent_ops, :end_user, :expires
...
* `persistentOps` 可指定上传完成后，需要自动执行哪些数据处理。这是因为有些数据处理操作（比如音视频转码）比较慢，如果不进行预转可能第一次访问的时候效果不理想，预转可以很大程度改善这一点。
